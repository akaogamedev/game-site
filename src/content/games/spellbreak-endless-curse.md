---
title: スペルブレイク！
catch: 迫る呪壁を魔法で食い止めろ。マナで球と呪文を強化するエンドレス・ブロック崩し。
category: unityroom
unityroomNo: 14
genre: アクション
platforms: [unityroom]
releaseDate: 2026-09-06
color: "#5b3e96"
cover: /images/games/spellbreak/cover.jpg
links:
  - label: unityroomで遊ぶ
    url: https://unityroom.com/games/spellbreak-endless-curse
featured: true
dialogue:
  - speaker: onaga
    text: "スペルブレイク！って、魔法でブロック崩しするゲームなん?"
  - speaker: akao
    text: "魔力球を打ち返してブロックを壊し、集めたマナで5種類の魔法を強化するエンドレスのブロック崩しだよ。"
  - speaker: onaga
    text: "呪壁っていうのが時間と一緒に近づいてくるんやね。"
  - speaker: akao
    text: "そう。迫る壁に飲み込まれる前に、ラリーを続けて魔法を育て、ハイスコアを狙うんだ。"
  - speaker: onaga
    text: "魔法はどんなふうに使うん?"
  - speaker: akao
    text: "マナが溜まるたびに3枚のカードから魔法を選んで強化するんだ。炎は3ヒットごとに周囲を爆発させ、雷は8ヒットごとに近くのブロックへ連鎖する。氷は呪壁が進むとき危険な列を凍らせ、幻影球はラリーで増え、魔導ビームは杖からビームを放つ。どの魔法もLv5まで育てられるよ。"
---

## 特徴

- 迫る呪壁から生き残るエンドレスの魔法ブロック崩し
- ブロックを壊してマナを集め、3枚の魔法カードから強化を選択
- 炎の爆発、連鎖する雷、氷の呪縛、幻影球、魔導ビームの5種類の魔法
- 魔法を12回強化すると通常球が2個に増加
- ラリーと魔法の組み合わせでハイスコアを狙うランキング対応

## 操作方法

- **移動**: マウス移動 / ← →キー / A・Dキー
- **開始・発射**: Space / Enter / 左クリック
- **魔法カード**: 矢印キーまたはWASDで選択してEnterで決定。クリックまたは1・2・3キーでも選択可能
- **一時停止・再開**: Esc / 画面右上の一時停止ボタン
- **やり直し**: Rキー

魔力球を落とさずラリーを伸ばし、強くなり続ける呪壁を突破して最高スコアを目指します。
