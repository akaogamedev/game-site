...
export const koGames: Record<string, GameTranslation> = {
  ...
  numphabet: {
    title: "Numphabet",
    catch: "영단어를 숫자로 푸는 60초 단어 퍼즐.",
    genre: "단어 퍼즐",
    dialogue: [
      { speaker: "onaga", text: "Numphabet… 이거 어떻게 읽어?" },
      { speaker: "akao", text: "'넘파벳'이야. number와 alphabet을 합친 말이지. A=1, B=2, Z=26까지. 영단어가 나오면 숫자 카드로 그 단어를 만들어." },
      { speaker: "onaga", text: "그럼 CAT이면 3, 1, 20만 누르면 되는 거야? 쉽네." },
      { speaker: "akao", text: "보드에는 꼭 가짜 카드가 섞여 있고, 대부분 정답과 1~2밖에 차이 안 나. NORMAL부터는 9+5처럼 계산해야 하는 카드와 1이 Z가 되는 거꾸로 카드도 나와." },
      { speaker: "onaga", text: "거꾸로? 머리가 꼬이겠네… 친구랑도 할 수 있어?" },
      { speaker: "akao", text: "친구 코드로 친구를 추가하면 같은 단어로 겨루는 5라운드 선승 대전을 할 수 있어. 하루 5문제 데일리 챌린지도 있으니 조금씩 이어가 봐." },
    ],
    features: [
      "A=1 … Z=26. 단어의 글자가 되는 숫자 카드를 왼쪽부터 순서대로 탭",
      "60초 스코어 어택. 20연속 정답이면 콤보 배율이 ×2.00까지 상승",
      "EASY·NORMAL·HARD 3단계. NORMAL은 9+5 같은 계산 카드와 1이 Z가 되는 거꾸로 카드, HARD는 계산한 뒤 거꾸로 세는 조합 카드가 추가",
      "모든 보드에 가짜 카드가 4장 이상, 대부분 정답과 1~2 차이",
      "해금한 난이도에 맞춰 매일 5문제가 나오는 데일리 챌린지(메달과 연속 기록)",
      "친구 코드로 친구를 추가하고 비공개 방에서 5라운드 선승 친구 대전(스태미나 소모 없음)",
      "난이도별 최고 기록과 랭킹, 상세 기록, 공유할 수 있는 결과 이미지",
    ],
    notes: [
      "무료 테마 Gumdrop 외에도 화면, 로고, 음악, 효과음이 모두 바뀌는 단품 구매 테마 Prism Glass와 Candy Sky가 있습니다. 무료 다운로드, 앱 내 구매 포함, 구독 없음. iPhone·iPad 지원, 일본어·영어 지원.",
    ],
  },
  "time-reversi": {
    title: "타임 리버시",
  ...
  "bachitto-pose": {
    "title": "Bachitto Pose! (착! 포즈!)",
    "catch": "벽의 구멍에 맞춰 포즈를 취하세요! 틀리면 물속으로 풍덩!",
    "genre": "액션",
    "dialogue": [
      {
        "speaker": "onaga",
        "text": "저 구멍을 정말 통과할 수 있을까? 벽이 가까워지니 조급해져!"
      },
      {
        "speaker": "akao",
        "text": "포즈가 딱 맞아 벽을 통과하는 순간이 짜릿해. 다음 벽에서는 선택지도 바뀌니 방심할 수 없어."
      },
      {
        "speaker": "onaga",
        "text": "성공하면 으스대고 싶겠다! 하지만 틀리면 바로 물속이네…"
      },
      {
        "speaker": "akao",
        "text": "박수와 물보라 덕분에 예능 프로그램 같아. 한 장만 더 통과하고 싶어서 다시 도전하게 돼."
      }
    ],
    "features": [
      "다가오는 벽의 구멍을 보고 네 가지 중 같은 포즈 선택",
      "벽마다 포즈 선택지가 바뀌고 연속 통과할수록 속도 증가",
      "성공하면 「バチッ！」, 실패하면 뒤로 날아가 물속으로 추락",
      "드럼 롤과 박수가 만드는 예능 분위기, 연속 통과 기록 온라인 랭킹"
    ],
    "controls": [
      {
        "label": "포즈 선택",
        "text": "↑ / ← / ↓ / → 또는 화면 아래 포즈 카드 클릭"
      },
      {
        "label": "시작 / 재도전",
        "text": "Space / Enter 또는 화면 버튼"
      },
      {
        "label": "타이틀로 돌아가기",
        "text": "결과 화면에서 Esc"
      ...
      {
        "label": "소리 전환",
        "text": "M 또는 오른쪽 위 소리 버튼"
      }
    ],
    "notes": [
      "벽이 도착하기 전에 구멍과 같은 포즈를 골라 연속 통과 기록에 도전하세요.",
      "PC 브라우저를 권장합니다. 게임 내 표시는 일본어입니다."
    ]
  },
  "tsukkomi-dojo": {
    "title": "츳코미 도장",
    "catch": "파트너의 보케에 딱 맞는 타이밍으로 츳코미! 목표는 '천하제일 츳코미'.",
    "genre": "코미디",
    "dialogue": [
      {
        "speaker": "onaga",
        "text": "츳코미 도장? 츳코미 수행이라도 하는 거야?"
      },
      {
        "speaker": "akao",
        "text": "파트너 '마사루'의 보케에 내 목소리로 츳코미를 거는 만자이 게임이야. 마사루는 편의점 점원 흉내를 내면서 어딘가 엇나간 접객을 해."
      },
      {
        "speaker": "onaga",
        "text": "츳코미라면 내 특기지!"
      ...
      {
        "speaker": "akao",
        "text": "객석이 얼마나 달아오를지는 AI가 츳코미를 판정해서 정해."
      },
      {
        "speaker": "onaga",
        "text": "AI가 심사위원이라고! 내 츳코미라면 객석이 빵빵 터질걸!"
      }
    ],
    "features": [
      "파트너 '마사루'의 보케에 내 목소리로 츳코미를 거는 만자이 게임",
      "츳코미의 정확도를 AI가 판정. 이상한 점을 정확히 짚고 비유나 재치를 더하면 더 높은 평가",
      "잘 받아치면 객석이 빵 터지고, 빗나가면 정적……. 요세 무대에서 펼쳐지는 콩트 만자이",
      "보케 7개가 끝나면 심사위원이 종합 평가. 목표는 칭호 '천하제일 츳코미'"
    ],
    "controls": [
      {
        "label": "목소리로 플레이",
        "text": "보케 후 마이크에 대고 츳코미. unityroom 페이지의 '別タブで開く'(새 탭에서 열기)로 열고 마이크를 허용하세요"
      },
      {
        "label": "텍스트로 플레이",
        "text": "보케 후 나타나는 입력란에 츳코미를 입력하고 Enter. 입력을 시작한 시각으로 타이밍을 잽니다"
      },
      {
        "label": "다시 플레이",
        "text": "결과 화면의 'もう一度' 또는 'タイトルへ'"
      }
    ],
    "notes": [
      "한 판에 약 3분. 소리를 낼 수 없는 분은 텍스트로도 츳코미할 수 있어요.",
      "츳코미 판정에 AI를 사용합니다. 츳코미 텍스트(음성은 인식된 텍스트)가 판정 서버로 전송됩니다.",
      "PC의 Chrome을 권장합니다. 게임 내 표시는 일본어이며, 음성 인식은 일본어만 지원합니다."
    ]
  },
};
